use std::{panic::catch_unwind, ptr::NonNull, rc::Rc};

mod aesthetic_rules;
mod gen;
use gen::*;
use rand::prelude::*;
use tidy_tree::{geometry::Coord, BasicLayout, Layout, Link, Node, TidyLayout, WeakLink};
...
pub fn test_partial_layout(layout: &mut dyn Layout) {
    ...
    for _ in 0..10 {
        ...
        let mut tree = tree.borrow_mut();
        layout.layout(&mut tree);
        let mut nodes: Vec<Link> = vec![];
        tree.pre_order_traversal(|node| nodes.push(node.me_rc()));
        for _ in 0..100 {
...
pub fn align_partial_layout_with_full_layout(layout: &mut dyn Layout) {
    ...
    for _i in 0..10 {
        ...
        for times in 0..100 {
            let new_node = insert_random_node(&mut rng, &nodes);
            let changed_node = change_random_node(&mut rng, &nodes);
            layout.partial_layout(&mut tree, &[new_node.clone(), changed_node]);
            // let partial_str = tree.str();
            let partial_x: Vec<Coord> = tree.iter().map(|node| node.borrow().x).collect();
            layout.layout(&mut tree);
            let full_x: Vec<Coord> = tree.iter().map(|node| node.borrow().x).collect();
            for i in 0..partial_x.len() {
                if (full_x[i] - partial_x[i]).abs() > 1e-6 {
                    let v = new_node.upgrade();
                    println!("NEW_NODE: {}", v.unwrap().borrow().str());
...
fn change_random_node(rng: &mut StdRng, nodes: &[Link]) -> WeakLink {
    ...
    Rc::downgrade(&nodes[node_index])
}

fn insert_random_node(rng: &mut StdRng, nodes: &[Link]) -> WeakLink {
    let node_index = rng.gen_range(0..nodes.len());
